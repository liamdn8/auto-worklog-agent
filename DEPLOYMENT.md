...
2. **Python 3** with `python-xlib` package
...
```bash
# Install python-xlib (required for window detection)
pip3 install python-xlib

# Verify installation
python3 -c "from Xlib import display; print('python-xlib installed successfully')"
```
...
d = display.Display()
atom = d.get_atom('_NET_ACTIVE_WINDOW')
wp = d.screen().root.get_full_property(atom, X.AnyPropertyType)
print('Window ID:', wp.value[0] if wp else 'None')
"
```
